...
import com.yingke.widget.pulltorefresh.internal.CustomLoadingLayout;
import com.yingke.widget.pulltorefresh.internal.FlipLoadingLayout;
import com.yingke.widget.pulltorefresh.internal.LoadingLayout;
import com.yingke.widget.pulltorefresh.internal.RotateLoadingLayout;
import com.yingke.widget.pulltorefresh.internal.Utils;
import com.yingke.widget.pulltorefresh.internal.ViewCompat;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
    ...
    //使得LoadingLayout停留下拉区域的上方不动
    private boolean mKeepLoadingLayout = false;
    //使得LoadingLayout停留下拉区域的中间
    private boolean mKeepLoadingCenterLayout = true;
    //onSizeChange 时是否刷新页面
    private boolean mCanResize = true;


    // 是否为手动下拉刷新
    private boolean mIsManualPullToRefresh;
    ...
    Handler mDissMissTipViewHandler = new Handler(Looper.getMainLooper());
    Runnable dissmissRunnable = new Runnable() {
        @Override
        public void run() {
            dissmissTipView();
        }
    ...
    public void showTipView(CharSequence message) {
        if (mHeaderLayout != null) {
            mHeaderLayout.showTipView(message);
            smoothScrollTo(-mHeaderLayout.getTipViewHeight());
            mDissMissTipViewHandler.removeCallbacks(dissmissRunnable);
            mDissMissTipViewHandler.postDelayed(dissmissRunnable, 1300);
        }
    }

    public void dissmissTipView() {
        if (mHeaderLayout != null) {
            mHeaderLayout.dissmissTipView();
            smoothScrollTo(0);
            // 刷新结束
            onRefreshComplete();
        }
    ...
    public void setKeepHeaderLayout(boolean value) {
        this.mKeepLoadingLayout = value;
    }

    public void setKeepLoadingCenterLayout(boolean value) {
        this.mKeepLoadingCenterLayout = value;
    }
    ...
    protected final void setHeaderScroll(int value) {
        ...
        //keep loading layout in position
        if (mKeepLoadingLayout && mHeaderLayout instanceof CustomLoadingLayout) {
            CustomLoadingLayout sll = (CustomLoadingLayout) mHeaderLayout;
            int layoutHeight = sll.getInnerLayoutHeight();
            int marginBottom = -value - layoutHeight;
            if (DEBUG) {
                Log.d(LOG_TAG, "margin bottom =" + marginBottom);
            }
            if (marginBottom > 0) {
                sll.setInnerLayoutMarginBottom(marginBottom);
            }
        } else if (mKeepLoadingCenterLayout && mHeaderLayout instanceof CustomLoadingLayout) {
            CustomLoadingLayout sll = (CustomLoadingLayout) mHeaderLayout;
            int layoutHeight = sll.getInnerLayoutHeight();
            int marginBottom = (-value - layoutHeight) / 2;
            if (DEBUG) {
                Log.d(LOG_TAG, "margin bottom =" + marginBottom);
            }
            if (marginBottom > 0) {
     ...
     * @return
     */
    public boolean isManualPullToRefresh() {
        return mIsManualPullToRefresh;
    }
     ...
     * 手动下拉刷新结束
     */
    protected void resetManualPullToRefresh() {
        this.mIsManualPullToRefresh = false;
    }
}
